...
contract('XDCValidator', (accounts) => {
    it('Become a candidate', async () => {
        let validator = await XDCValidator.new((new BigNumber(5 * 10 ** 18)).toString(), 99, 100, 100)
    ...
    it('A candidate withdraw ', async () => {
        let validator = await XDCValidator.new((new BigNumber(5 * 10 ** 18)).toString(), 99, 0, 0)
        await validator.propose(accounts[0], nodeUrl, { from : accounts[0], value: 5.0 * 10 ** 18 })
