...
const nodeId = 'XDC-efbb5dccf38ab2'

contract('XDCValidator', (accounts) => {
    it('Become a candidate', async () => {
        let validator = await XDCValidator.new([], [], null, (new BigNumber(5 * 10 ** 18)).toString(), 99, 100, 100)
    ...
    it('A candidate withdraw', async () => {
        let validator = await XDCValidator.new([], [], null, (new BigNumber(5 * 10 ** 18)).toString(), 99, 0, 0)
        await validator.propose(accounts[0], nodeId, { from : accounts[0], value: 5.0 * 10 ** 18 })
    ...
    it('A voter withdraw before block number', async () => {
        let validator = await XDCValidator.new([], [], null, (new BigNumber(5 * 10 ** 18)).toString(), 99, 0, 100)
        await validator.propose(accounts[0], nodeId, { from : accounts[0], value: 5.0 * 10 ** 18 })
